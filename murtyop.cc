...
struct MurtyFunctor<Eigen::ThreadPoolDevice> {
  void operator()(const Eigen::ThreadPoolDevice& d, const int64_t k, const int64_t n, const int64_t m, const float* in, int64_t* out) {
    ...
    struct cpu_futhark_i64_2d* fut_out = cpu_futhark_new_i64_2d(ctx, out, m, k); 
    cpu_futhark_entry_main(ctx, &fut_out, fut_in, k);
    cpu_futhark_context_sync(ctx);
    cpu_futhark_values_i64_2d(ctx, fut_out, out);
    cpu_futhark_free_f32_2d(ctx, fut_in);
    cpu_futhark_free_i64_2d(ctx, fut_out);
...
struct MurtyFunctor<Eigen::GpuDevice> {
  void operator()(const Eigen::GpuDevice& d, const int64_t k, const int64_t n, const int64_t m, const float* in, int64_t* out) {
    ...
    struct gpu_futhark_i64_2d* fut_out = gpu_futhark_new_i64_2d(ctx, out, m, k); 
    gpu_futhark_entry_main(ctx, &fut_out, fut_in, k);
    gpu_futhark_context_sync(ctx);
    gpu_futhark_values_i64_2d(ctx, fut_out, out);
    gpu_futhark_free_f32_2d(ctx, fut_in);
    gpu_futhark_free_i64_2d(ctx, fut_out);
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    MurtyFunctor<Device>()(
        ...
        static_cast<int64_t*>(output_tensor->data())
    );
  }
};
